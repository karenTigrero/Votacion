...
def detect(request):
    ...
    while(True):
        ...
        for(x,y,w,h) in faces:
            ...
            if conf<35:
                userId = getId
                cv2.putText(img, "Detectado c:",(x,y+h), font, 2, (0,255,0),2)
            else:
        ...
        if(cv2.waitKey(1) == ord('q')):
            ...
            cam.release()
            cv2.destroyAllWindows()
            return redirect('/votante/voto/'+str(userId))
...
def detectImage(request):
    ...
    return redirect('/votante/voto/'+str(pred[0]))

def listadol(request):
    model = Lista
